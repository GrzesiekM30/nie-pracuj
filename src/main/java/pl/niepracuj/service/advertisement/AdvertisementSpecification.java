...
public class AdvertisementSpecification implements Specification<Advertisement> {
    ...
    private Predicate technologyNameEquals(Root<Advertisement> root, CriteriaQuery<?> query, CriteriaBuilder criteriaBuilder) {
    ...
    private Predicate salaryFromLessThanOrEqualTo(Root<Advertisement> root, CriteriaQuery<?> query, CriteriaBuilder criteriaBuilder) {
        return nonNull(criteriaDto.getSalaryTo()) ?
                criteriaBuilder.lessThanOrEqualTo(root.get("salaryFrom"), criteriaDto.getSalaryTo()) :
                alwaysTruePredicate(criteriaBuilder);
    }

    private Predicate salaryToLessThanOrEqualTo(Root<Advertisement> root, CriteriaQuery<?> query, CriteriaBuilder criteriaBuilder) {
